...
const persistanceConfig = {
  ...
  whitelist: (persist ? ['orm'] : []).concat(
    Object.keys(reducerConfig)
      .map(k => reducerConfig[k].persist && k)
      .filter(r => r),
  ),
  transforms: Object.keys(reducerConfig)
    .map(k => Array.isArray(reducerConfig[k].persist) && createFilter(k, reducerConfig[k].persist))
    .filter(r => r),
};

...
export function create(formData) {
  return (dispatch, getState) => {
    const { type, addresses, updateDefaultAddress } = formData;
    if (!type) {
      throw new Error('You must provide a wallet type');
    }
    const addedAddresses = Object.values(addresses).filter(a => a.enabled);
    if (addressesAreInvalid(addedAddresses)) {
    ...
    addedAddresses.forEach((address) => {
      dispatch({ type: addressActions.CREATE_ADDRESS, payload: { ...address, keystore: id, updateDefaultAddress } });
    });
  };
}
...
export function update(formData, oldData) {
  return (dispatch, getState) => {
    ...
    // validate
    if (addressesAreInvalid(addresses.filter(a => a.enabled))) {
      throw Error('You must provide a name for all addresses');
    }

...
export default class TransactionModal extends Component {
  ...
  componentWillMount() {
    ...
    // fetch(ethGasApi)
    //   .then(resp => resp.json())
    //   .then(({ fast, fastest, safeLow, average }) => this.setState({
    //     gasPrice: average / 10,
    //     gasStation: {
    //       safeLow: safeLow / 10,
    //       average: average / 10,
    ...
    //     loading: false,
    //   }))
    //   .catch(e => console.error('Error in fetching eth gas price', e));

    // TOO: replace with something from Eth Gas Station or Digix's own api
    this.setState({
      gasPrice: 20,
      gasStation: {
        safeLow: 1,
  ...
  handleValidation(txData, web3) {
    ...
    if (handleValidation) {
      ...
      return handleValidation({ txData, formData }, web3).catch(() => {
        ...
        // throw new Error('Failure'); // comment out due uncaught error. no handler or not needed?
      });
    }
    return null;
  }
  handleSubmit(formData, web3) {
    ...
    new Promise(resolve => setTimeout(resolve, 10)) // time for UI update
      ...
      .then((txHash) => {
        if (!txHash) {
          throw Error('Transaction was not published!');
        }
        this.setState({ formData, txHash, loading: false, broadcast: new Date() });
        if (onBroadcast) {
          onBroadcast({ formData, txHash });
        }
      })
  ...
  render() {
    ...
    const { txHash, open, failure, gasPrice, gasStation } = this.state;
    const networkId = data.networkId || (network && network.id) || undefined;
    if (!open) {
      return (
        <span tabIndex={0} role="button" onClick={() => this.setState({ open: true })}>
          {trigger}
        </span>
      );
    }
    ...
    return (
      <TransactionModalContainer
        {...{
          ...this.props,
          ...this.state,
          data: { ...data, gasPrice: data.gasPrice || (gasPrice && gasPrice * 1e9) || 20e9, networkId, gasStation },
          ...
          content: (props) => {
            if (txHash) {
              return this.renderTracker(props);
            }

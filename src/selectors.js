...
export const getNetworksWithTokens = createSelector(
  ...
  createOrmSelector(schema, session =>
    session.Network.all()
      ...
      .map(network => ({
        ...network.ref,
        tokens: network.tokens.all().toRefArray(),
      })),
  ),
);
...
export const getDefaultNetworks = createSelector(
  ormSelector,
  createOrmSelector(schema, session =>
    session.Network.filter({ default: true })
      .toModelArray()
      .map(({ id }) => id),
  ),
);
...
export const getKeystoreTypes = createSelector(
  ormSelector,
  createOrmSelector(schema, session => session.KeystoreType.all().toRefArray()),
);
...
export const getDefaultAddressAndKeyStore = createSelector(
  ormSelector,
  createOrmSelector(schema, (session) => {
    const { defaultAddress } = session.Session.first() || {};
    return (
      defaultAddress && {
        ...defaultAddress.ref,
        keystore: {
          ...defaultAddress.keystore.ref,
          type: { ...defaultAddress.keystore.type.ref },
        },
      }
    );
  }),
...
export const getSigningMessageModalData = createSelector(
  ormSelector,
  createOrmSelector(schema, session => (session.Session.first() || {}).signingMsgModalData),
);
